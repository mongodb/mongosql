...
func TestTranslatePredicate(t *testing.T) {
	...
	runTests := func(tests []test) {
		...
		translator := evaluator.NewPushDownTranslator(
			...
			log.NewComponentLogger("TestTranslatePredicate", log.GlobalLogger()),
		)
	...
	Convey("Subject: TranslatePredicate", t, func() {
		tests := []test{
			...
			test{"t IS NOT TRUE", `{"$and":[{"t":{"$ne":true}},{"t":{"$ne":null}}]}`},
			test{"t IS FALSE", `{"t":false}`},
			test{"t IS NOT FALSE", `{"$and":[{"t":{"$ne":false}},{"t":{"$ne":null}}]}`},
		...
		partialTests := []partialTest{
			...
			partialTest{"a", `{"a":{"$ne":null}}`, `a`, evaluator.NewSQLColumnExpr(1, "test", tableTwoName, "a", schema.SQLInt, schema.MongoInt)},
			partialTest{"a = 3 AND a < b", `{"a":3}`, "a < b", evaluator.NewSQLLessThanExpr(evaluator.NewSQLColumnExpr(1, "test", tableTwoName, "a", schema.SQLInt, schema.MongoInt),
				evaluator.NewSQLColumnExpr(1, "test", tableTwoName, "b", schema.SQLInt, schema.MongoInt))},
			partialTest{"a = 3 AND a < b AND b = 4", `{"$and":[{"a":3},{"b":4}]}`, "a < b", evaluator.NewSQLLessThanExpr(evaluator.NewSQLColumnExpr(1, "test", tableTwoName, "a", schema.SQLInt, schema.MongoInt),
				evaluator.NewSQLColumnExpr(1, "test", tableTwoName, "b", schema.SQLInt, schema.MongoInt))},
			partialTest{"a < b AND a = 3", `{"a":3}`, "a < b", evaluator.NewSQLLessThanExpr(evaluator.NewSQLColumnExpr(1, "test", tableTwoName, "a", schema.SQLInt, schema.MongoInt),
				evaluator.NewSQLColumnExpr(1, "test", tableTwoName, "b", schema.SQLInt, schema.MongoInt))},
			partialTest{"NOT (a = 3 AND a < b)", `{"$and":[{"a":{"$ne":3}},{"a":{"$ne":null}}]}`, "NOT a < b",
				evaluator.NewSQLNotExpr(evaluator.NewSQLLessThanExpr(evaluator.NewSQLColumnExpr(1, "test", tableTwoName, "a", schema.SQLInt, schema.MongoInt),
					evaluator.NewSQLColumnExpr(1, "test", tableTwoName, "b", schema.SQLInt, schema.MongoInt)))},
		}
...
func TestExprNoPushdown(t *testing.T) {
	...
	runTests := func(tests []test, schemaName []byte, tableName string) {
		...
		translator := evaluator.NewPushDownTranslator(
			func(_ ...uint8) bool { return true },
			createFieldNameLookup(db),
			log.NewComponentLogger("TestExprNoPushDown", log.GlobalLogger()),
		)
		for _, t := range tests {
			Convey(fmt.Sprintf("%q should not be pushed down", t.sql), func() {
				e, err := evaluator.GetSQLExpr(schema, dbOne, tableName, t.sql)
				So(err, ShouldBeNil)
	...
	Convey("Subject: ExprNoPushdown", t, func() {
		...
		tests := []test{
			...
			test{"length(a)"},
			test{"dayname(a)"},
			test{"greatest(a, 'test')"},
			test{"greatest(a, 'test', 4)"},
			test{"least(a, 'test')"},
			test{"least(a, 'test', 4)"},
			test{"s + s"},
			test{"s - s"},
...
func TestTranslateExpr(t *testing.T) {
	...
	runTests := func(tests []test) {
		...
		translator := evaluator.NewPushDownTranslator(
			func(_ ...uint8) bool { return true },
			createFieldNameLookup(db),
			log.NewComponentLogger("TestTranslateExpr", log.GlobalLogger()),
		)
		for _, t := range tests {
	...
	Convey("Subject: TranslateExpr", t, func() {
		...
		tests := []test{
			test{"a / b", `{"$let":{"in":{"$cond":[{"$eq":["$$right",0]},null,{"$divide":["$$left","$$right"]}]},"vars":{"left":"$a","right":"$b"}}}`},
			test{"a div b", `{"$let":{"in":{"$cond":[{"$eq":["$$right",0]},null,{"$trunc":[{"$divide":["$$left","$$right"]}]}]},"vars":{"left":"$a","right":"$b"}}}`},
			test{"t = 0", `{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$ifNull":["$$left",null]},null]},{"$eq":[{"$ifNull":["$$right",null]},null]}]},null,{"$eq":["$$left","$$right"]}]},"vars":{"left":"$t","right":{"$literal":false}}}}`},
			test{"abs(a)", `{"$abs":"$a"}`},
			test{"adddate(g, INTERVAL 10 day)", `{"$let":{"in":{"$cond":[{"$eq":[{"$ifNull":["$$date",null]},null]},null,{"$add":["$$date",864000000]}]},"vars":{"date":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$g"},"date"]}]},"then":"$g"},{"case":{"$or":[{"$eq":[{"$type":"$g"},"int"]},{"$eq":[{"$type":"$g"},"decimal"]},{"$eq":[{"$type":"$g"},"long"]},{"$eq":[{"$type":"$g"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",1000000]}]},"then":{"$add":[{"$multiply":["$g",1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":["$g",1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",100000000]}]},"then":{"$multiply":["$g",1000000]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",1000000000000]}]},"then":{"$add":["$g",{"$cond":[{"$lt":[{"$divide":["$g",10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",100000000000000]}]},"then":"$g"}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$g"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00","$g"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$g","T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":["$g","T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":["$g"," "]},1]},""]}]}}}}]}}],"default":null}}}}}`},
			test{"adddate(g, null)", `{"$literal":null}`},
			test{"adddate(date_format(g, '%Y-01-01'), interval 0 second)", `{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]}},"date"]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]}},"int"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]}},"decimal"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]}},"long"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]}},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},1000000]}]},"then":{"$add":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},100000000]}]},"then":{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},1000000]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},1000000000000]}]},"then":{"$add":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},{"$cond":[{"$lt":[{"$divide":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},100000000000000]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]}},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00",{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]}]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},"T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]},"T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01"}}]}," "]},1]},""]}]}}}}]}}],"default":null}}`},
			test{"adddate(date_format(g, '%Y-01-01 00:00:00'), interval 0 second)", `{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]}},"date"]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]}},"int"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]}},"decimal"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]}},"long"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]}},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},1000000]}]},"then":{"$add":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},100000000]}]},"then":{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},1000000]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},1000000000000]}]},"then":{"$add":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},{"$cond":[{"$lt":[{"$divide":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},100000000000000]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]}},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00",{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]}]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},"T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]},"T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-01-01 00:00:00"}}]}," "]},1]},""]}]}}}}]}}],"default":null}}`},
			test{"adddate(date_format(g, '%Y-%m-01'), interval 0 second)", `{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]}},"date"]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]}},"int"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]}},"decimal"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]}},"long"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]}},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},1000000]}]},"then":{"$add":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},100000000]}]},"then":{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},1000000]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},1000000000000]}]},"then":{"$add":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},{"$cond":[{"$lt":[{"$divide":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},100000000000000]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]}},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00",{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]}]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},"T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]},"T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01"}}]}," "]},1]},""]}]}}}}]}}],"default":null}}`},
			test{"adddate(date_format(g, '%Y-%m-01 00:00:00'), interval 0 second)", `{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]}},"date"]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]}},"int"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]}},"decimal"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]}},"long"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]}},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},1000000]}]},"then":{"$add":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},100000000]}]},"then":{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},1000000]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},1000000000000]}]},"then":{"$add":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},{"$cond":[{"$lt":[{"$divide":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},100000000000000]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]}},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00",{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]}]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},"T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]},"T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-01 00:00:00"}}]}," "]},1]},""]}]}}}}]}}],"default":null}}`},
			test{"adddate(date_format(g, '%Y-%m-%d'), interval 0 second)", `{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]}},"date"]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]}},"int"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]}},"decimal"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]}},"long"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]}},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},1000000]}]},"then":{"$add":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},100000000]}]},"then":{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},1000000]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},1000000000000]}]},"then":{"$add":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},{"$cond":[{"$lt":[{"$divide":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},100000000000000]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]}},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00",{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]}]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},"T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]},"T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d"}}]}," "]},1]},""]}]}}}}]}}],"default":null}}`},
			test{"adddate(date_format(g, '%Y-%m-%d 00:00:00'), interval 0 second)", `{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]}},"date"]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]}},"int"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]}},"decimal"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]}},"long"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]}},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},1000000]}]},"then":{"$add":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},100000000]}]},"then":{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},1000000]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},1000000000000]}]},"then":{"$add":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},{"$cond":[{"$lt":[{"$divide":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},100000000000000]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]}},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00",{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]}]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},"T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]},"T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d 00:00:00"}}]}," "]},1]},""]}]}}}}]}}],"default":null}}`},
			test{"adddate(date_format(g, '%Y-%m-%d %H:00:00'), interval 0 second)", `{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]}},"date"]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]}},"int"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]}},"decimal"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]}},"long"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]}},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},1000000]}]},"then":{"$add":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},100000000]}]},"then":{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},1000000]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},1000000000000]}]},"then":{"$add":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},{"$cond":[{"$lt":[{"$divide":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},100000000000000]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]}},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00",{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]}]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},"T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]},"T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:00:00"}}]}," "]},1]},""]}]}}}}]}}],"default":null}}`},
			test{"adddate(date_format(g, '%Y-%m-%d %H:%i:00'), interval 0 second)", `{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]}},"date"]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]}},"int"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]}},"decimal"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]}},"long"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]}},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},1000000]}]},"then":{"$add":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},100000000]}]},"then":{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},1000000]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},1000000000000]}]},"then":{"$add":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},{"$cond":[{"$lt":[{"$divide":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},100000000000000]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]}},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00",{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]}]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},"T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]},"T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:00"}}]}," "]},1]},""]}]}}}}]}}],"default":null}}`},
			test{"adddate(date_format(g, '%Y-%m-%d %H:%i:%s'), interval 0 second)", `{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"date"]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"int"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"decimal"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"long"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000]}]},"then":{"$add":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},100000000]}]},"then":{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000000000]}]},"then":{"$add":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},{"$cond":[{"$lt":[{"$divide":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},100000000000000]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00",{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},"T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},"T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}," "]},1]},""]}]}}}}]}}],"default":null}}`},
			test{"adddate(date_format(g, '%Y-%m-%d %H:%i:%s'), interval 10 second)", `{"$let":{"in":{"$cond":[{"$eq":[{"$ifNull":["$$date",null]},null]},null,{"$add":["$$date",10000]}]},"vars":{"date":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"date"]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"int"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"decimal"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"long"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000]}]},"then":{"$add":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},100000000]}]},"then":{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000000000]}]},"then":{"$add":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},{"$cond":[{"$lt":[{"$divide":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},100000000000000]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00",{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},"T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},"T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}," "]},1]},""]}]}}}}]}}],"default":null}}}}}`},
			test{"adddate(date_format(g, '%Y-%m-%d %H:%i:%s'), interval -10 minute)", `{"$let":{"in":{"$cond":[{"$eq":[{"$ifNull":["$$date",null]},null]},null,{"$add":["$$date",-600000]}]},"vars":{"date":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"date"]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"int"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"decimal"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"long"]},{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000]}]},"then":{"$add":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},100000000]}]},"then":{"$multiply":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},1000000000000]}]},"then":{"$add":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},{"$cond":[{"$lt":[{"$divide":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},0]},{"$lt":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},100000000000000]}]},"then":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00",{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},"T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]},"T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$dateToString":{"date":"$g","format":"%Y-%m-%d %H:%M:%S"}}]}," "]},1]},""]}]}}}}]}}],"default":null}}}}}`},
			test{"ceil(a)", `{"$ceil":"$a"}`},
			...
			test{"datediff(g, '2000-09-01 13:29:18')", `{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$ifNull":["$g",null]},null]},{"$eq":[{"$ifNull":["2000-09-01T00:00:00Z",null]},null]}]},null,{"$cond":[{"$or":[{"$gt":["$$days",106751]},{"$lt":["$$days",-106751]}]},{"$cond":[{"$gt":[{"$divide":[{"$subtract":["$g","2000-09-01T00:00:00Z"]},86400000]},106751]},106751,-106751]},"$$days"]}]},"vars":{"days":{"$divide":[{"$subtract":["$g","2000-09-01T00:00:00Z"]},86400000]}}}}`},
			test{"datediff(g, h)", `{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$ifNull":["$g",null]},null]},{"$eq":[{"$ifNull":["$h",null]},null]}]},null,{"$cond":[{"$or":[{"$gt":["$$days",106751]},{"$lt":["$$days",-106751]}]},{"$cond":[{"$gt":[{"$divide":[{"$subtract":["$g","$h"]},86400000]},106751]},106751,-106751]},"$$days"]}]},"vars":{"days":{"$divide":[{"$subtract":["$g","$h"]},86400000]}}}}`},
			test{"date_add(g, INTERVAL 10 day)", `{"$let":{"in":{"$cond":[{"$eq":[{"$ifNull":["$$date",null]},null]},null,{"$add":["$$date",864000000]}]},"vars":{"date":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$g"},"date"]}]},"then":"$g"},{"case":{"$or":[{"$eq":[{"$type":"$g"},"int"]},{"$eq":[{"$type":"$g"},"decimal"]},{"$eq":[{"$type":"$g"},"long"]},{"$eq":[{"$type":"$g"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",1000000]}]},"then":{"$add":[{"$multiply":["$g",1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":["$g",1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",100000000]}]},"then":{"$multiply":["$g",1000000]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",1000000000000]}]},"then":{"$add":["$g",{"$cond":[{"$lt":[{"$divide":["$g",10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",100000000000000]}]},"then":"$g"}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$g"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00","$g"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$g","T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":["$g","T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":["$g"," "]},1]},""]}]}}}}]}}],"default":null}}}}}`},
			...
			test{"date_sub(g, INTERVAL 10 day)", `{"$let":{"in":{"$cond":[{"$eq":[{"$ifNull":["$$date",null]},null]},null,{"$add":["$$date",-864000000]}]},"vars":{"date":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$g"},"date"]}]},"then":"$g"},{"case":{"$or":[{"$eq":[{"$type":"$g"},"int"]},{"$eq":[{"$type":"$g"},"decimal"]},{"$eq":[{"$type":"$g"},"long"]},{"$eq":[{"$type":"$g"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",1000000]}]},"then":{"$add":[{"$multiply":["$g",1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":["$g",1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",100000000]}]},"then":{"$multiply":["$g",1000000]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",1000000000000]}]},"then":{"$add":["$g",{"$cond":[{"$lt":[{"$divide":["$g",10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",100000000000000]}]},"then":"$g"}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$g"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00","$g"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$g","T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":["$g","T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":["$g"," "]},1]},""]}]}}}}]}}],"default":null}}}}}`},
			...
			test{"greatest(a, 2)", `{"$cond":[{"$eq":[{"$ifNull":["$a",null]},null]},null,{"$max":["$a",{"$literal":2}]}]}`},
			test{"hour(g)", `{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$hour":"$g"}]}`},
			test{"if(a, 2, 3)", `{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$ifNull":["$$expr",null]},null]},{"$eq":["$$expr",0]},{"$eq":["$$expr",false]}]},{"$literal":3},{"$literal":2}]},"vars":{"expr":"$a"}}}`},
			...
			test{"nullif(a, 1)", `{"$let":{"in":{"$cond":[{"$eq":[{"$ifNull":["$$expr",null]},null]},null,{"$cond":[{"$eq":["$$expr",{"$literal":1}]},null,"$$expr"]}]},"vars":{"expr":"$a"}}}`},
			test{"quarter(g)", `{"$let":{"in":{"$cond":[{"$eq":[{"$ifNull":["$$date",null]},null]},null,{"$arrayElemAt":[[1,1,1,2,2,2,3,3,3,4,4,4],{"$subtract":[{"$month":"$$date"},1]}]}]},"vars":{"date":"$g"}}}`},
			...
			test{"subdate(g, INTERVAL 10 day)", `{"$let":{"in":{"$cond":[{"$eq":[{"$ifNull":["$$date",null]},null]},null,{"$add":["$$date",-864000000]}]},"vars":{"date":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$g"},"date"]}]},"then":"$g"},{"case":{"$or":[{"$eq":[{"$type":"$g"},"int"]},{"$eq":[{"$type":"$g"},"decimal"]},{"$eq":[{"$type":"$g"},"long"]},{"$eq":[{"$type":"$g"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",1000000]}]},"then":{"$add":[{"$multiply":["$g",1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":["$g",1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",100000000]}]},"then":{"$multiply":["$g",1000000]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",1000000000000]}]},"then":{"$add":["$g",{"$cond":[{"$lt":[{"$divide":["$g",10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",100000000000000]}]},"then":"$g"}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$g"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00","$g"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$g","T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":["$g","T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":["$g"," "]},1]},""]}]}}}}]}}],"default":null}}}}}`},
			...
			test{"weekday(g)", `{"$let":{"in":{"$cond":[{"$eq":[{"$ifNull":["$$date",null]},null]},null,{"$mod":[{"$add":[{"$mod":[{"$subtract":[{"$dayOfWeek":"$$date"},2]},7]},7]},7]}]},"vars":{"date":"$g"}}}`},
			...
			test{"sum(a)", `{"$sum":"$a"}`},
			test{"sum(a < 1)", `{"$sum":{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$ifNull":["$$left",null]},null]},{"$eq":[{"$ifNull":["$$right",null]},null]}]},null,{"$lt":["$$left","$$right"]}]},"vars":{"left":"$a","right":{"$literal":1}}}}}`},
			...
			test{"case when a > 1 then 'gt' else 'lt' end", `{"$cond":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$ifNull":["$$left",null]},null]},{"$eq":[{"$ifNull":["$$right",null]},null]}]},null,{"$gt":["$$left","$$right"]}]},"vars":{"left":"$a","right":{"$literal":1}}}},{"$literal":"gt"},{"$literal":"lt"}]}`},
			...
			test{"date(s)", `{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$s"},"date"]}]},"then":"$s"},{"case":{"$or":[{"$eq":[{"$type":"$s"},"int"]},{"$eq":[{"$type":"$s"},"decimal"]},{"$eq":[{"$type":"$s"},"long"]},{"$eq":[{"$type":"$s"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]}]},{"$dateFromParts":{"day":"$$day","month":"$$month","year":"$$year"}},null]},"vars":{"day":{"$mod":["$$num",100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"year":{"$trunc":{"$divide":["$$num",10000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",1000000]}]},"then":{"$add":["$s",{"$cond":[{"$lt":[{"$divide":["$s",10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",100000000]}]},"then":"$s"},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",1000000000000]}]},"then":{"$add":[{"$trunc":{"$divide":["$s",1000000]}},{"$cond":[{"$lt":[{"$divide":[{"$trunc":{"$divide":["$s",1000000]}},10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",100000000000000]}]},"then":{"$trunc":{"$divide":["$s",1000000]}}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$s"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00","$s"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmed"},6]},null,{"$dateFromString":{"dateString":{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$joined"},6]},{"$eq":["-",{"$substrCP":["$$joined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$joined",0,2]},"70"]},"20","19"]},"$$joined"]},"$$joined"]},"vars":{"joined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmed","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmed"}]}}}}}}}}}}}}]},"vars":{"trimmed":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$s","T"]},0]}," "]},0]}}}}]}}],"default":null}}`},
			test{"timestampdiff(month, s, s)", `{"$let":{"in":{"$let":{"in":{"$let":{"in":{"$let":{"in":{"$switch":{"branches":[{"case":{"$lt":["$$output",{"$literal":0}]},"then":{"$add":["$$output",{"$cond":[{"$or":[{"$gt":["$$day2","$$day1"]},{"$gt":["$$hour2","$$hour1"]},{"$gt":["$$minute2","$$minute1"]},{"$gt":["$$second2","$$second1"]},{"$gt":["$$millisecond2","$$millisecond1"]}]},{"$literal":1},{"$literal":0}]}]}},{"case":{"$gt":["$$output",{"$literal":0}]},"then":{"$subtract":["$$output",{"$cond":[{"$or":[{"$gt":["$$day1","$$day2"]},{"$gt":["$$hour1","$$hour2"]},{"$gt":["$$minute1","$$minute2"]},{"$gt":["$$second1","$$second2"]},{"$gt":["$$millisecond1","$$millisecond2"]}]},{"$literal":1},{"$literal":0}]}]}}],"default":{"$literal":0}}},"vars":{"output":{"$add":[{"$multiply":[{"$subtract":["$$year2","$$year1"]},12]},{"$subtract":["$$month2","$$month1"]}]}}}},"vars":{"output":{"$add":[{"$multiply":[{"$subtract":["$$year2","$$year1"]},12]},{"$subtract":["$$month2","$$month1"]}]}}}},"vars":{"day1":{"$dayOfMonth":["$$timestampArg1"]},"day2":{"$dayOfMonth":["$$timestampArg2"]},"hour1":{"$hour":["$$timestampArg1"]},"hour2":{"$hour":["$$timestampArg2"]},"millisecond1":{"$millisecond":["$$timestampArg1"]},"millisecond2":{"$millisecond":["$$timestampArg2"]},"minute1":{"$minute":["$$timestampArg1"]},"minute2":{"$minute":["$$timestampArg2"]},"month1":{"$month":["$$timestampArg1"]},"month2":{"$month":["$$timestampArg2"]},"second1":{"$second":["$$timestampArg1"]},"second2":{"$second":["$$timestampArg2"]},"year1":{"$year":["$$timestampArg1"]},"year2":{"$year":["$$timestampArg2"]}}}},"vars":{"timestampArg1":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$s"},"date"]}]},"then":"$s"},{"case":{"$or":[{"$eq":[{"$type":"$s"},"int"]},{"$eq":[{"$type":"$s"},"decimal"]},{"$eq":[{"$type":"$s"},"long"]},{"$eq":[{"$type":"$s"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",1000000]}]},"then":{"$add":[{"$multiply":["$s",1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":["$s",1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",100000000]}]},"then":{"$multiply":["$s",1000000]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",1000000000000]}]},"then":{"$add":["$s",{"$cond":[{"$lt":[{"$divide":["$s",10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",100000000000000]}]},"then":"$s"}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$s"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00","$s"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$s","T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":["$s","T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":["$s"," "]},1]},""]}]}}}}]}}],"default":null}},"timestampArg2":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$s"},"date"]}]},"then":"$s"},{"case":{"$or":[{"$eq":[{"$type":"$s"},"int"]},{"$eq":[{"$type":"$s"},"decimal"]},{"$eq":[{"$type":"$s"},"long"]},{"$eq":[{"$type":"$s"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",1000000]}]},"then":{"$add":[{"$multiply":["$s",1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":["$s",1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",100000000]}]},"then":{"$multiply":["$s",1000000]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",1000000000000]}]},"then":{"$add":["$s",{"$cond":[{"$lt":[{"$divide":["$s",10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",100000000000000]}]},"then":"$s"}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$s"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00","$s"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$s","T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":["$s","T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":["$s"," "]},1]},""]}]}}}}]}}],"default":null}}}}}`},
			test{"timestampdiff(microsecond, s, s)", `{"$let":{"in":{"$trunc":[{"$divide":[{"$subtract":["$$timestampArg2","$$timestampArg1"]},0.001]}]},"vars":{"timestampArg1":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$s"},"date"]}]},"then":"$s"},{"case":{"$or":[{"$eq":[{"$type":"$s"},"int"]},{"$eq":[{"$type":"$s"},"decimal"]},{"$eq":[{"$type":"$s"},"long"]},{"$eq":[{"$type":"$s"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",1000000]}]},"then":{"$add":[{"$multiply":["$s",1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":["$s",1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",100000000]}]},"then":{"$multiply":["$s",1000000]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",1000000000000]}]},"then":{"$add":["$s",{"$cond":[{"$lt":[{"$divide":["$s",10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",100000000000000]}]},"then":"$s"}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$s"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00","$s"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$s","T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":["$s","T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":["$s"," "]},1]},""]}]}}}}]}}],"default":null}},"timestampArg2":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$s"},"date"]}]},"then":"$s"},{"case":{"$or":[{"$eq":[{"$type":"$s"},"int"]},{"$eq":[{"$type":"$s"},"decimal"]},{"$eq":[{"$type":"$s"},"long"]},{"$eq":[{"$type":"$s"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",1000000]}]},"then":{"$add":[{"$multiply":["$s",1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":["$s",1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",100000000]}]},"then":{"$multiply":["$s",1000000]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",1000000000000]}]},"then":{"$add":["$s",{"$cond":[{"$lt":[{"$divide":["$s",10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",100000000000000]}]},"then":"$s"}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$s"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00","$s"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$s","T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":["$s","T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":["$s"," "]},1]},""]}]}}}}]}}],"default":null}}}}}`},
			test{"timestamp(s)", `{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$s"},"date"]}]},"then":"$s"},{"case":{"$or":[{"$eq":[{"$type":"$s"},"int"]},{"$eq":[{"$type":"$s"},"decimal"]},{"$eq":[{"$type":"$s"},"long"]},{"$eq":[{"$type":"$s"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]},{"$and":[{"$gte":["$$hour",0]},{"$lt":["$$hour",24]}]},{"$and":[{"$gte":["$$minute",0]},{"$lt":["$$minute",60]}]},{"$and":[{"$gte":["$$second",0]},{"$lt":["$$second",60]}]}]},{"$dateFromParts":{"day":"$$day","hour":"$$hour","millisecond":"$$millisecond","minute":"$$minute","month":"$$month","second":"$$second","year":"$$year"}},null]},"vars":{"day":{"$mod":[{"$trunc":{"$divide":["$$num",1000000]}},100]},"hour":{"$mod":[{"$trunc":{"$divide":["$$num",10000]}},100]},"millisecond":{"$trunc":{"$multiply":[{"$mod":["$$num",1]},1000]}},"minute":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100000000]}},100]},"second":{"$trunc":{"$mod":["$$num",100]}},"year":{"$trunc":{"$divide":["$$num",10000000000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",1000000]}]},"then":{"$add":[{"$multiply":["$s",1000000]},{"$cond":[{"$lt":[{"$divide":[{"$multiply":["$s",1000000]},10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",100000000]}]},"then":{"$multiply":["$s",1000000]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",1000000000000]}]},"then":{"$add":["$s",{"$cond":[{"$lt":[{"$divide":["$s",10000000000]},70]},20000000000000,19000000000000]}]}},{"case":{"$and":[{"$gte":["$s",0]},{"$lt":["$s",100000000000000]}]},"then":"$s"}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$s"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00 00:00:00","$s"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmedDate"},6]},null,{"$dateFromString":{"dateString":{"$concat":[{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$dateJoined"},6]},{"$eq":["-",{"$substrCP":["$$dateJoined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$dateJoined",0,2]},"70"]},"20","19"]},"$$dateJoined"]},"$$dateJoined"]},"vars":{"dateJoined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedDate","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedDate"}]}}}}}}}}}},{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},".","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmedTime","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmedTime"}]}}}}}}}]}}}]},"vars":{"trimmedDate":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$s","T"]},0]}," "]},0]},"trimmedTime":{"$ifNull":[{"$arrayElemAt":[{"$split":["$s","T"]},1]},{"$ifNull":[{"$arrayElemAt":[{"$split":["$s"," "]},1]},""]}]}}}}]}}],"default":null}}`},
			...
			test{"last_day(a)", `{"$let":{"in":{"$let":{"in":{"$dateFromParts":{"day":{"$switch":{"branches":[{"case":{"$eq":["$$month",2]},"then":{"$cond":[{"$let":{"in":{"$or":[{"$and":[{"$eq":[{"$mod":["$$val",4]},0]},{"$ne":[{"$mod":["$$val",100]},0]}]},{"$eq":[{"$mod":["$$val",400]},0]}]},"vars":{"val":"$$year"}}},29,28]}},{"case":{"$eq":["$$month",4]},"then":30},{"case":{"$eq":["$$month",6]},"then":30},{"case":{"$eq":["$$month",9]},"then":30},{"case":{"$eq":["$$month",11]},"then":30}],"default":31}},"month":"$$month","year":"$$year"}},"vars":{"month":{"$month":["$$date"]},"year":{"$year":["$$date"]}}}},"vars":{"date":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$a"},"date"]}]},"then":"$a"},{"case":{"$or":[{"$eq":[{"$type":"$a"},"int"]},{"$eq":[{"$type":"$a"},"decimal"]},{"$eq":[{"$type":"$a"},"long"]},{"$eq":[{"$type":"$a"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]}]},{"$dateFromParts":{"day":"$$day","month":"$$month","year":"$$year"}},null]},"vars":{"day":{"$mod":["$$num",100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"year":{"$trunc":{"$divide":["$$num",10000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",1000000]}]},"then":{"$add":["$a",{"$cond":[{"$lt":[{"$divide":["$a",10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",100000000]}]},"then":"$a"},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",1000000000000]}]},"then":{"$add":[{"$trunc":{"$divide":["$a",1000000]}},{"$cond":[{"$lt":[{"$divide":[{"$trunc":{"$divide":["$a",1000000]}},10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",100000000000000]}]},"then":{"$trunc":{"$divide":["$a",1000000]}}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$a"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00","$a"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmed"},6]},null,{"$dateFromString":{"dateString":{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$joined"},6]},{"$eq":["-",{"$substrCP":["$$joined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$joined",0,2]},"70"]},"20","19"]},"$$joined"]},"$$joined"]},"vars":{"joined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmed","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmed"}]}}}}}}}}}}}}]},"vars":{"trimmed":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$a","T"]},0]}," "]},0]}}}}]}}],"default":null}}}}}`},
			test{"substring_index(s,s,a)", `{"$let":{"in":{"$let":{"in":{"$reduce":{"in":{"$cond":[{"$eq":["$$value",null]},"$$this",{"$concat":["$$value","$$delim","$$this"]}]},"initialValue":null,"input":"$$split"}},"vars":{"split":{"$slice":[{"$split":["$s","$$delim"]},{"$cond":[{"$lt":["$a",0]},{"$ceil":[{"$subtract":["$a",0.5]}]},{"$floor":[{"$add":["$a",0.5]}]}]}]}}}},"vars":{"delim":"$s"}}}`},
			...
			test{"from_days(a)", `{"$let":{"in":{"$cond":[{"$eq":[{"$ifNull":["$$arg",null]},null]},null,{"$cond":[{"$or":[{"$gt":["$$arg",3652499]},{"$lt":["$$arg",366]}]},"0000-00-00",{"$add":["0000-01-01T00:00:00Z",{"$multiply":[{"$cond":[{"$lt":["$a",0]},{"$ceil":[{"$subtract":["$a",0.5]}]},{"$floor":[{"$add":["$a",0.5]}]}]},86400000]}]}]}]},"vars":{"arg":"$a"}}}`},
			test{"replace(s,s,s)", `{"$let":{"in":{"$reduce":{"in":{"$cond":[{"$eq":["$$value",null]},"$$this",{"$concat":["$$value","$s","$$this"]}]},"initialValue":null,"input":"$$split"}},"vars":{"split":{"$split":["$s","$s"]}}}}`},
			test{"week(g)", `{"$let":{"in":{"$let":{"in":{"$cond":[{"$eq":[{"$ifNull":["$$date",null]},null]},null,{"$week":"$$date"}]},"vars":{"year":{"$year":["$$date"]}}}},"vars":{"date":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$g"},"date"]}]},"then":"$g"},{"case":{"$or":[{"$eq":[{"$type":"$g"},"int"]},{"$eq":[{"$type":"$g"},"decimal"]},{"$eq":[{"$type":"$g"},"long"]},{"$eq":[{"$type":"$g"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]}]},{"$dateFromParts":{"day":"$$day","month":"$$month","year":"$$year"}},null]},"vars":{"day":{"$mod":["$$num",100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"year":{"$trunc":{"$divide":["$$num",10000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",1000000]}]},"then":{"$add":["$g",{"$cond":[{"$lt":[{"$divide":["$g",10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",100000000]}]},"then":"$g"},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",1000000000000]}]},"then":{"$add":[{"$trunc":{"$divide":["$g",1000000]}},{"$cond":[{"$lt":[{"$divide":[{"$trunc":{"$divide":["$g",1000000]}},10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",100000000000000]}]},"then":{"$trunc":{"$divide":["$g",1000000]}}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$g"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00","$g"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmed"},6]},null,{"$dateFromString":{"dateString":{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$joined"},6]},{"$eq":["-",{"$substrCP":["$$joined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$joined",0,2]},"70"]},"20","19"]},"$$joined"]},"$$joined"]},"vars":{"joined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmed","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmed"}]}}}}}}}}}}}}]},"vars":{"trimmed":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$g","T"]},0]}," "]},0]}}}}]}}],"default":null}}}}}`},
			test{"week(g, 0)", `{"$let":{"in":{"$let":{"in":{"$cond":[{"$eq":[{"$ifNull":["$$date",null]},null]},null,{"$week":"$$date"}]},"vars":{"year":{"$year":["$$date"]}}}},"vars":{"date":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$g"},"date"]}]},"then":"$g"},{"case":{"$or":[{"$eq":[{"$type":"$g"},"int"]},{"$eq":[{"$type":"$g"},"decimal"]},{"$eq":[{"$type":"$g"},"long"]},{"$eq":[{"$type":"$g"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]}]},{"$dateFromParts":{"day":"$$day","month":"$$month","year":"$$year"}},null]},"vars":{"day":{"$mod":["$$num",100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"year":{"$trunc":{"$divide":["$$num",10000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",1000000]}]},"then":{"$add":["$g",{"$cond":[{"$lt":[{"$divide":["$g",10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",100000000]}]},"then":"$g"},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",1000000000000]}]},"then":{"$add":[{"$trunc":{"$divide":["$g",1000000]}},{"$cond":[{"$lt":[{"$divide":[{"$trunc":{"$divide":["$g",1000000]}},10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$g",0]},{"$lt":["$g",100000000000000]}]},"then":{"$trunc":{"$divide":["$g",1000000]}}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$g"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00","$g"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmed"},6]},null,{"$dateFromString":{"dateString":{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$joined"},6]},{"$eq":["-",{"$substrCP":["$$joined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$joined",0,2]},"70"]},"20","19"]},"$$joined"]},"$$joined"]},"vars":{"joined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmed","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmed"}]}}}}}}}}}}}}]},"vars":{"trimmed":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$g","T"]},0]}," "]},0]}}}}]}}],"default":null}}}}}`},
			test{"week(a,7)", `{"$let":{"in":{"$let":{"in":{"$let":{"in":{"$cond":[{"$ne":["$$output",0]},"$$output",{"$let":{"in":{"$let":{"in":{"$let":{"in":{"$trunc":[{"$add":[{"$divide":[{"$subtract":["$$date",{"$add":[{"$dateFromParts":{"day":1,"month":1,"year":"$$year"}},{"$multiply":[{"$mod":[{"$add":[{"$subtract":[7,"$$jan1DayOfWeek"]},2]},7]},86400000]}]}]},604800000]},1]}]},"vars":{"jan1DayOfWeek":{"$dayOfWeek":[{"$dateFromParts":{"day":1,"month":1,"year":"$$year"}}]}}}},"vars":{"year":{"$year":["$$date"]}}}},"vars":{"date":{"$dateFromParts":{"day":31,"month":12,"year":{"$subtract":["$$year",1]}}}}}}]},"vars":{"output":{"$let":{"in":{"$trunc":[{"$add":[{"$divide":[{"$subtract":["$$date",{"$add":[{"$dateFromParts":{"day":1,"month":1,"year":"$$year"}},{"$multiply":[{"$mod":[{"$add":[{"$subtract":[7,"$$jan1DayOfWeek"]},2]},7]},86400000]}]}]},604800000]},1]}]},"vars":{"jan1DayOfWeek":{"$dayOfWeek":[{"$dateFromParts":{"day":1,"month":1,"year":"$$year"}}]}}}}}}},"vars":{"year":{"$year":["$$date"]}}}},"vars":{"date":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$a"},"date"]}]},"then":"$a"},{"case":{"$or":[{"$eq":[{"$type":"$a"},"int"]},{"$eq":[{"$type":"$a"},"decimal"]},{"$eq":[{"$type":"$a"},"long"]},{"$eq":[{"$type":"$a"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]}]},{"$dateFromParts":{"day":"$$day","month":"$$month","year":"$$year"}},null]},"vars":{"day":{"$mod":["$$num",100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"year":{"$trunc":{"$divide":["$$num",10000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",1000000]}]},"then":{"$add":["$a",{"$cond":[{"$lt":[{"$divide":["$a",10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",100000000]}]},"then":"$a"},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",1000000000000]}]},"then":{"$add":[{"$trunc":{"$divide":["$a",1000000]}},{"$cond":[{"$lt":[{"$divide":[{"$trunc":{"$divide":["$a",1000000]}},10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",100000000000000]}]},"then":{"$trunc":{"$divide":["$a",1000000]}}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$a"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00","$a"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmed"},6]},null,{"$dateFromString":{"dateString":{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$joined"},6]},{"$eq":["-",{"$substrCP":["$$joined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$joined",0,2]},"70"]},"20","19"]},"$$joined"]},"$$joined"]},"vars":{"joined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmed","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmed"}]}}}}}}}}}}}}]},"vars":{"trimmed":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$a","T"]},0]}," "]},0]}}}}]}}],"default":null}}}}}`},
			test{"week(a,5)", `{"$let":{"in":{"$let":{"in":{"$let":{"in":{"$trunc":[{"$add":[{"$divide":[{"$subtract":["$$date",{"$add":[{"$dateFromParts":{"day":1,"month":1,"year":"$$year"}},{"$multiply":[{"$mod":[{"$add":[{"$subtract":[7,"$$jan1DayOfWeek"]},2]},7]},86400000]}]}]},604800000]},1]}]},"vars":{"jan1DayOfWeek":{"$dayOfWeek":[{"$dateFromParts":{"day":1,"month":1,"year":"$$year"}}]}}}},"vars":{"year":{"$year":["$$date"]}}}},"vars":{"date":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$a"},"date"]}]},"then":"$a"},{"case":{"$or":[{"$eq":[{"$type":"$a"},"int"]},{"$eq":[{"$type":"$a"},"decimal"]},{"$eq":[{"$type":"$a"},"long"]},{"$eq":[{"$type":"$a"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]}]},{"$dateFromParts":{"day":"$$day","month":"$$month","year":"$$year"}},null]},"vars":{"day":{"$mod":["$$num",100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"year":{"$trunc":{"$divide":["$$num",10000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",1000000]}]},"then":{"$add":["$a",{"$cond":[{"$lt":[{"$divide":["$a",10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",100000000]}]},"then":"$a"},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",1000000000000]}]},"then":{"$add":[{"$trunc":{"$divide":["$a",1000000]}},{"$cond":[{"$lt":[{"$divide":[{"$trunc":{"$divide":["$a",1000000]}},10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",100000000000000]}]},"then":{"$trunc":{"$divide":["$a",1000000]}}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$a"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00","$a"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmed"},6]},null,{"$dateFromString":{"dateString":{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$joined"},6]},{"$eq":["-",{"$substrCP":["$$joined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$joined",0,2]},"70"]},"20","19"]},"$$joined"]},"$$joined"]},"vars":{"joined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmed","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmed"}]}}}}}}}}}}}}]},"vars":{"trimmed":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$a","T"]},0]}," "]},0]}}}}]}}],"default":null}}}}}`},
			test{"yearweek(a,5)", `{"$let":{"in":{"$let":{"in":{"$let":{"in":{"$let":{"in":{"$add":[{"$multiply":["$$newYear",100]},"$$week"]},"vars":{"newYear":{"$switch":{"branches":[{"case":{"$eq":["$$week",1]},"then":{"$cond":[{"$eq":["$$month",12]},{"$add":["$$year",1]},"$$year"]}},{"case":{"$eq":["$$week",52]},"then":{"$cond":[{"$eq":["$$month",1]},{"$subtract":["$$year",1]},"$$year"]}},{"case":{"$eq":["$$week",53]},"then":{"$cond":[{"$eq":["$$month",1]},{"$subtract":["$$year",1]},"$$year"]}}],"default":"$$year"}}}}},"vars":{"week":{"$let":{"in":{"$let":{"in":{"$let":{"in":{"$cond":[{"$ne":["$$output",0]},"$$output",{"$let":{"in":{"$let":{"in":{"$let":{"in":{"$trunc":[{"$add":[{"$divide":[{"$subtract":["$$date",{"$add":[{"$dateFromParts":{"day":1,"month":1,"year":"$$year"}},{"$multiply":[{"$mod":[{"$add":[{"$subtract":[7,"$$jan1DayOfWeek"]},2]},7]},86400000]}]}]},604800000]},1]}]},"vars":{"jan1DayOfWeek":{"$dayOfWeek":[{"$dateFromParts":{"day":1,"month":1,"year":"$$year"}}]}}}},"vars":{"year":{"$year":["$$date"]}}}},"vars":{"date":{"$dateFromParts":{"day":31,"month":12,"year":{"$subtract":["$$year",1]}}}}}}]},"vars":{"output":{"$let":{"in":{"$trunc":[{"$add":[{"$divide":[{"$subtract":["$$date",{"$add":[{"$dateFromParts":{"day":1,"month":1,"year":"$$year"}},{"$multiply":[{"$mod":[{"$add":[{"$subtract":[7,"$$jan1DayOfWeek"]},2]},7]},86400000]}]}]},604800000]},1]}]},"vars":{"jan1DayOfWeek":{"$dayOfWeek":[{"$dateFromParts":{"day":1,"month":1,"year":"$$year"}}]}}}}}}},"vars":{"year":{"$year":["$$date"]}}}},"vars":{"date":"$$date"}}}}}},"vars":{"month":{"$month":["$$date"]},"year":{"$year":["$$date"]}}}},"vars":{"date":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$a"},"date"]}]},"then":"$a"},{"case":{"$or":[{"$eq":[{"$type":"$a"},"int"]},{"$eq":[{"$type":"$a"},"decimal"]},{"$eq":[{"$type":"$a"},"long"]},{"$eq":[{"$type":"$a"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]}]},{"$dateFromParts":{"day":"$$day","month":"$$month","year":"$$year"}},null]},"vars":{"day":{"$mod":["$$num",100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"year":{"$trunc":{"$divide":["$$num",10000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",1000000]}]},"then":{"$add":["$a",{"$cond":[{"$lt":[{"$divide":["$a",10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",100000000]}]},"then":"$a"},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",1000000000000]}]},"then":{"$add":[{"$trunc":{"$divide":["$a",1000000]}},{"$cond":[{"$lt":[{"$divide":[{"$trunc":{"$divide":["$a",1000000]}},10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",100000000000000]}]},"then":{"$trunc":{"$divide":["$a",1000000]}}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$a"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00","$a"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmed"},6]},null,{"$dateFromString":{"dateString":{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$joined"},6]},{"$eq":["-",{"$substrCP":["$$joined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$joined",0,2]},"70"]},"20","19"]},"$$joined"]},"$$joined"]},"vars":{"joined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmed","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmed"}]}}}}}}}}}}}}]},"vars":{"trimmed":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$a","T"]},0]}," "]},0]}}}}]}}],"default":null}}}}}`},
			test{"yearweek(a,7)", `{"$let":{"in":{"$let":{"in":{"$let":{"in":{"$let":{"in":{"$add":[{"$multiply":["$$newYear",100]},"$$week"]},"vars":{"newYear":{"$switch":{"branches":[{"case":{"$eq":["$$week",1]},"then":{"$cond":[{"$eq":["$$month",12]},{"$add":["$$year",1]},"$$year"]}},{"case":{"$eq":["$$week",52]},"then":{"$cond":[{"$eq":["$$month",1]},{"$subtract":["$$year",1]},"$$year"]}},{"case":{"$eq":["$$week",53]},"then":{"$cond":[{"$eq":["$$month",1]},{"$subtract":["$$year",1]},"$$year"]}}],"default":"$$year"}}}}},"vars":{"week":{"$let":{"in":{"$let":{"in":{"$let":{"in":{"$cond":[{"$ne":["$$output",0]},"$$output",{"$let":{"in":{"$let":{"in":{"$let":{"in":{"$trunc":[{"$add":[{"$divide":[{"$subtract":["$$date",{"$add":[{"$dateFromParts":{"day":1,"month":1,"year":"$$year"}},{"$multiply":[{"$mod":[{"$add":[{"$subtract":[7,"$$jan1DayOfWeek"]},2]},7]},86400000]}]}]},604800000]},1]}]},"vars":{"jan1DayOfWeek":{"$dayOfWeek":[{"$dateFromParts":{"day":1,"month":1,"year":"$$year"}}]}}}},"vars":{"year":{"$year":["$$date"]}}}},"vars":{"date":{"$dateFromParts":{"day":31,"month":12,"year":{"$subtract":["$$year",1]}}}}}}]},"vars":{"output":{"$let":{"in":{"$trunc":[{"$add":[{"$divide":[{"$subtract":["$$date",{"$add":[{"$dateFromParts":{"day":1,"month":1,"year":"$$year"}},{"$multiply":[{"$mod":[{"$add":[{"$subtract":[7,"$$jan1DayOfWeek"]},2]},7]},86400000]}]}]},604800000]},1]}]},"vars":{"jan1DayOfWeek":{"$dayOfWeek":[{"$dateFromParts":{"day":1,"month":1,"year":"$$year"}}]}}}}}}},"vars":{"year":{"$year":["$$date"]}}}},"vars":{"date":"$$date"}}}}}},"vars":{"month":{"$month":["$$date"]},"year":{"$year":["$$date"]}}}},"vars":{"date":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$a"},"date"]}]},"then":"$a"},{"case":{"$or":[{"$eq":[{"$type":"$a"},"int"]},{"$eq":[{"$type":"$a"},"decimal"]},{"$eq":[{"$type":"$a"},"long"]},{"$eq":[{"$type":"$a"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]}]},{"$dateFromParts":{"day":"$$day","month":"$$month","year":"$$year"}},null]},"vars":{"day":{"$mod":["$$num",100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"year":{"$trunc":{"$divide":["$$num",10000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",1000000]}]},"then":{"$add":["$a",{"$cond":[{"$lt":[{"$divide":["$a",10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",100000000]}]},"then":"$a"},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",1000000000000]}]},"then":{"$add":[{"$trunc":{"$divide":["$a",1000000]}},{"$cond":[{"$lt":[{"$divide":[{"$trunc":{"$divide":["$a",1000000]}},10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",100000000000000]}]},"then":{"$trunc":{"$divide":["$a",1000000]}}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$a"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00","$a"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmed"},6]},null,{"$dateFromString":{"dateString":{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$joined"},6]},{"$eq":["-",{"$substrCP":["$$joined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$joined",0,2]},"70"]},"20","19"]},"$$joined"]},"$$joined"]},"vars":{"joined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmed","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmed"}]}}}}}}}}}}}}]},"vars":{"trimmed":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$a","T"]},0]}," "]},0]}}}}]}}],"default":null}}}}}`},
		}
	...
	Convey("Subject: TranslateExpr with SQLValue", t, func() {
		...
		sqlValueTests := []sqlValueTest{
			sqlValueTest{evaluator.SQLTrue, `{"$literal":true}`},
			sqlValueTest{evaluator.SQLFalse, `{"$literal":false}`},
			sqlValueTest{evaluator.SQLFloat(1.1), `{"$literal":1.1}`},
			sqlValueTest{evaluator.SQLInt(11), `{"$literal":11}`},
			sqlValueTest{evaluator.SQLUint32(32), `{"$literal":32}`},
			sqlValueTest{evaluator.SQLVarchar("vc"), `{"$literal":"vc"}`},
			sqlValueTest{evaluator.SQLNull, `{"$literal":null}`},
			sqlValueTest{evaluator.SQLDate{t}, `{"$literal":"2012-12-07T12:15:30.918273645Z"}`},
			sqlValueTest{evaluator.SQLTimestamp{t}, `{"$literal":"2012-12-07T12:15:30.918273645Z"}`},
		}
