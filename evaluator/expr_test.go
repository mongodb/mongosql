...
func TestTranslateExpr(t *testing.T) {
	...
	Convey("Subject: TranslateExpr", t, func() {
		...
		tests := []test{
			...
			test{"greatest(a, 2)", `{"$cond":[{"$or":[{"$eq":[{"$ifNull":["$a",null]},null]},{"$eq":[{"$ifNull":[{"$literal":2},null]},null]}]},null,{"$max":["$a",{"$literal":2}]}]}`},
			...
			test{"least(a, 2)", `{"$cond":[{"$or":[{"$eq":[{"$ifNull":["$a",null]},null]},{"$eq":[{"$ifNull":[{"$literal":2},null]},null]}]},null,{"$min":["$a",{"$literal":2}]}]}`},
			test{"least(a, 2, b)", `{"$cond":[{"$or":[{"$eq":[{"$ifNull":["$a",null]},null]},{"$eq":[{"$ifNull":[{"$literal":2},null]},null]},{"$eq":[{"$ifNull":["$b",null]},null]}]},null,{"$min":["$a",{"$literal":2},"$b"]}]}`},
