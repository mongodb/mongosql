...
func TestTranslateExpr(t *testing.T) {
	...
	Convey("Subject: TranslateExpr", t, func() {
		...
		tests := []test{
			...
			test{"adddate(date_format(g, '%Y-%m-%d %H:%i:%s'), interval 10 second)", `{"$cond":[{"$eq":[{"$ifNull":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$subtract":["$g",{"$millisecond":"$g"}]}]},null]},null]},null,{"$add":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$subtract":["$g",{"$millisecond":"$g"}]}]},10000]}]}`},
			test{"adddate(date_format(g, '%Y-%m-%d %H:%i:%s'), interval -10 minute)", `{"$cond":[{"$eq":[{"$ifNull":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$subtract":["$g",{"$millisecond":"$g"}]}]},null]},null]},null,{"$add":[{"$cond":[{"$eq":[{"$ifNull":["$g",null]},null]},null,{"$subtract":["$g",{"$millisecond":"$g"}]}]},-600000]}]}`},
			test{"ceil(a)", `{"$ceil":"$a"}`},
			...
			test{"sum(a)", `{"$sum":"$a"}`},
			test{"sum(a < 1)", `{"$sum":{"$cond":[{"$eq":[{"$ifNull":["$a",null]},null]},null,{"$lt":["$a",{"$literal":1}]}]}}`},
