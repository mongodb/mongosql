...
func TestTranslateExpr(t *testing.T) {
	...
	Convey("Subject: TranslateExpr", t, func() {
		...
		tests := []test{
			...
			test{"last_day(a)", `{"$let":{"in":{"$let":{"in":{"$dateFromParts":{"day":{"$switch":{"branches":[{"case":{"$eq":["$$month",2]},"then":{"$cond":[{"$eq":[{"$mod":["$$year",4]},0]},29,28]}},{"case":{"$eq":["$$month",4]},"then":30},{"case":{"$eq":["$$month",6]},"then":30},{"case":{"$eq":["$$month",9]},"then":30},{"case":{"$eq":["$$month",11]},"then":30}],"default":31}},"month":"$$month","year":"$$year"}},"vars":{"month":{"$month":["$$date"]},"year":{"$year":["$$date"]}}}},"vars":{"date":{"$switch":{"branches":[{"case":{"$or":[{"$eq":[{"$type":"$a"},"date"]}]},"then":"$a"},{"case":{"$or":[{"$eq":[{"$type":"$a"},"int"]},{"$eq":[{"$type":"$a"},"decimal"]},{"$eq":[{"$type":"$a"},"long"]},{"$eq":[{"$type":"$a"},"double"]}]},"then":{"$let":{"in":{"$let":{"in":{"$cond":[{"$and":[{"$and":[{"$gte":["$$year",0]},{"$lt":["$$year",10000]}]},{"$and":[{"$gte":["$$month",1]},{"$lt":["$$month",13]}]},{"$and":[{"$gte":["$$day",1]},{"$lt":["$$day",32]}]}]},{"$dateFromParts":{"day":"$$day","month":"$$month","year":"$$year"}},null]},"vars":{"day":{"$mod":["$$num",100]},"month":{"$mod":[{"$trunc":{"$divide":["$$num",100]}},100]},"year":{"$trunc":{"$divide":["$$num",10000]}}}}},"vars":{"num":{"$switch":{"branches":[{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",1000000]}]},"then":{"$add":["$a",{"$cond":[{"$lt":[{"$divide":["$a",10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",100000000]}]},"then":"$a"},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",1000000000000]}]},"then":{"$add":[{"$trunc":{"$divide":["$a",1000000]}},{"$cond":[{"$lt":[{"$divide":[{"$trunc":{"$divide":["$a",1000000]}},10000]},70]},20000000,19000000]}]}},{"case":{"$and":[{"$gte":["$a",0]},{"$lt":["$a",100000000000000]}]},"then":{"$trunc":{"$divide":["$a",1000000]}}}],"default":null}}}}}},{"case":{"$or":[{"$eq":[{"$type":"$a"},"string"]}]},"then":{"$cond":[{"$eq":["0000-00-00","$a"]},null,{"$let":{"in":{"$cond":[{"$lt":[{"$strLenCP":"$$trimmed"},6]},null,{"$dateFromString":{"dateString":{"$let":{"in":{"$cond":[{"$or":[{"$eq":[{"$strLenCP":"$$joined"},6]},{"$eq":["-",{"$substrCP":["$$joined",2,1]}]}]},{"$concat":[{"$cond":[{"$lt":[{"$substrCP":["$$joined",0,2]},"70"]},"20","19"]},"$$joined"]},"$$joined"]},"vars":{"joined":{"$reduce":{"in":{"$concat":["$$value","$$this"]},"initialValue":"","input":{"$map":{"as":"c","in":{"$cond":[{"$ne":[-1,{"$indexOfArray":[["!","\"","#",{"$literal":"$"},"%","\u0026","'","(",")","*","+",",","-",".","/",":",";","\u003c","=","\u003e","?","@","[","\\","]","^","_","` + "`" + `","{","|","}","~"],"$$c"]}]},"-","$$c"]},"input":{"$map":{"in":{"$substrCP":["$$trimmed","$$this",1]},"input":{"$range":[0,{"$strLenCP":"$$trimmed"}]}}}}}}}}}}}}]},"vars":{"trimmed":{"$arrayElemAt":[{"$split":[{"$arrayElemAt":[{"$split":["$a","T"]},0]}," "]},0]}}}}]}}],"default":null}}}}}`},
			...
			test{"replace(s,s,s)", `{"$let":{"in":{"$reduce":{"in":{"$cond":[{"$eq":["$$value",42]},"$$this",{"$concat":["$$value","$s","$$this"]}]},"initialValue":42,"input":"$$split"}},"vars":{"split":{"$split":["$s","$s"]}}}}`},
		}
