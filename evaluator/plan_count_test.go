...
func TestCountPlanStage(t *testing.T) {
	...
	t.Run("count stage should fetch the count from a table and return the right value", func(t *testing.T) {
		req := require.New(t)
		...
		values, err := bsonDToValues(1, "", "", bson.D{bson.DocElem{Name: "count(*)", Value: 263}})
		req.Nil(err, "failed to translate bsonD to values")
		...
		for iter.Next(row) {
			req.Equal(len(row.Data), len(expected[i]), "number of returned columns does not match number of expected columns")
			req.Equal(row.Data, expected[i], "returned data values does not match expected data values")
			row = &evaluator.Row{}
			i++
		}
		req.Equal(i, len(expected), "returned number of rows does not match expected number of rows")
