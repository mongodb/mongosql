...
func testDesugar(t *testing.T) {
	tcases := []struct {
		...
		},
		{
			...
			expected: "select (select 1 from DUAL) = (select * from foo) from DUAL",
		},
		{
			desc:     "reorder <=> subquery comparisons to non-subqueries",
			query:    "select (select * from foo) <=> 1 from DUAL",
		...
		},
		{
			...
			expected: "select * from (select (select 1 from DUAL) = (select * from foo) from DUAL)",
		},
		{
		...
		},
		{
			...
			expected: "select (select d, e, f from DUAL) < (select a, b, c from bar) from foo",
		},
		{
