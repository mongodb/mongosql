...
Follow the steps below to perform a public release of the BI Connector:
...
6. Update `mongodb-bi-downloads.json` to reflect the new release version and locations for each binary. e.g.```cat urls.json | jq -r 'to_entries[] | "\(.key)\t\(.value)\t\t\t"' | awk '{split($0,a," ")}{print a[1]}{name=system("basename " a[2])}' |  xargs -L2 sh -c 'sed -i .bak "s/$0/$1/" mongodb-bi-downloads.json' | sed -i .bak "s/VERSION/2.0.0-rc0/" mongodb-bi-downloads.json```.
