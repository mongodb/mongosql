...
type SaslServer struct {
	...
	state           C.mongosql_sspi_server_state
...
func (ss *SaslServer) Close() {
	C.mongosql_sspi_server_destroy(&ss.state)
}
...
func (ss *SaslServer) Start() error {
	...
	cspn := C.CString(ss.servicePrincipalName)
	defer C.free(unsafe.Pointer(cspn))

	status := C.mongosql_sspi_server_init(&ss.state, cspn)
	if status != C.SSPI_OK {
...
func (ss *SaslServer) Next(challenge []byte) ([]byte, error) {
	...
	if ss.supportComplete {
		buf = (C.PVOID)(unsafe.Pointer(&challenge[0]))
		bufLen = C.ULONG(len(challenge))
		status := C.mongosql_sspi_server_unwrap_msg(&ss.state, buf, bufLen, &outBuf, &outBufLen)
		if status != C.SSPI_OK {
			return nil, ss.getError("unable to unwrap authz")
		}

		bs := C.GoBytes(unsafe.Pointer(outBuf), C.int(outBufLen))
		ss.clientUsername = string(bs[4:])

		ss.done = true
	} else if ss.contextComplete {
		qop := []byte{1, 0, 0, 0} // quality of protection we support, which is none
		buf = (C.PVOID)(unsafe.Pointer(&qop[0]))
		bufLen = C.ULONG(len(qop))
		status := C.mongosql_sspi_server_wrap_msg(&ss.state, buf, bufLen, &outBuf, &outBufLen)
		if status != C.SSPI_OK {
			return nil, ss.getError("unable to wrap security support")
		}

		ss.supportComplete = true
	} else {
		if len(challenge) > 0 {
			buf = (C.PVOID)(unsafe.Pointer(&challenge[0]))
			bufLen = C.ULONG(len(challenge))
		...
		status := C.mongosql_sspi_server_negotiate(&ss.state, buf, bufLen, &outBuf, &outBufLen)
		switch status {
		case C.SSPI_OK:
			ss.contextComplete = true
...
func (ss *SaslServer) getError(prefix string) error {
	return getError(prefix, ss.state.status)
}
...
type SaslClient struct {
	...
	state           C.mongosql_sspi_client_state
	username        string
	contextComplete bool
	done            bool
}
...
func (sc *SaslClient) Close() {
	C.mongosql_sspi_client_destroy(&sc.state, &sc.server.state)
}

func (sc *SaslClient) Start() (string, []byte, error) {
	const mechName = "GSSAPI"

	cusername := C.CString(sc.server.clientUsername)
	defer C.free(unsafe.Pointer(cusername))

	status := C.mongosql_sspi_client_init(&sc.state, &sc.server.state, cusername)
...
func (sc *SaslClient) Next(challenge []byte) ([]byte, error) {
	...
	if sc.contextComplete {
		qop := append([]byte{1, 0, 0, 0}, []byte(sc.server.clientUsername)...)
		buf := (C.PVOID)(unsafe.Pointer(&qop[0]))
		bufLen := C.ULONG(len(qop))
		status := C.mongosql_sspi_client_wrap_msg(&sc.state, buf, bufLen, &outBuf, &outBufLen)
		if status != C.SSPI_OK {
		...
		status := C.mongosql_sspi_client_negotiate(
			&sc.state,
			cservicePrincipalName,
			buf,
			bufLen,
			&outBuf,
			&outBufLen,
...
func getError(prefix string, status C.SECURITY_STATUS) error {
	...
	case C.SEC_E_ALGORITHM_MISMATCH:
		s = "The client and server cannot communicate because " +
	...
	case C.SEC_E_BUFFER_TOO_SMALL:
		s = "The buffers supplied to the function are not large enough " +
			"to contain the information."
	case C.SEC_E_CANNOT_INSTALL:
		s = "The security package cannot initialize successfully and " +
	...
	case C.SEC_E_CONTEXT_EXPIRED:
		s = "The application is referencing a context that has already been closed. " +
			"A properly written application should not receive this error."
	case C.SEC_E_CROSSREALM_DELEGATION_FAILURE:
		s = "The server attempted to make a Kerberos-constrained delegation request " +
			"for a target outside the server's realm."
	case C.SEC_E_CRYPTO_SYSTEM_INVALID:
		s = "The cryptographic system or checksum function is not valid because " +
	...
	case C.SEC_E_DELEGATION_REQUIRED:
		s = "The requested operation cannot be completed. " +
			"The computer must be trusted for delegation"
	case C.SEC_E_DOWNGRADE_DETECTED:
		s = "The system detected a possible attempt to compromise security. " +
			"Verify that the server that authenticated you can be contacted."
	...
	case C.SEC_E_INCOMPLETE_CREDENTIALS:
		s = "The credentials supplied were not complete and could not be verified. " +
	...
	case C.SEC_E_ISSUING_CA_UNTRUSTED:
		s = "An untrusted certification authority (CA) was detected while processing " +
			"the smart card certificate used for authentication."
	case C.SEC_E_ISSUING_CA_UNTRUSTED_KDC:
		s = "An untrusted CA was detected while processing the domain controller " +
	...
	case C.SEC_E_NO_IP_ADDRESSES:
		s = "Unable to accomplish the requested task because the local computer " +
	...
	case C.SEC_E_NO_S4U_PROT_SUPPORT:
		s = "The Kerberos subsystem encountered an error. " +
			"A service for user protocol request was made against a domain controller " +
			"which does not support service for a user."
	case C.SEC_E_NO_TGT_REPLY:
		s = "The client is trying to negotiate a context and the server " +
			"requires a user-to-user connection"
	...
	case C.SEC_E_PKINIT_NAME_MISMATCH:
		s = "The client certificate does not contain a valid UPN or does not match " +
			"the client name in the logon request."
	...
	case C.SEC_E_REVOCATION_OFFLINE_C:
		s = "The revocation status of the smart card certificate used for authentication " +
			"could not be determined."
	case C.SEC_E_REVOCATION_OFFLINE_KDC:
		s = "The revocation status of the domain controller certificate used for " +
			"smart card authentication could not be determined. " +
			"The system event log contains additional information."
	case C.SEC_E_SECPKG_NOT_FOUND:
		s = "The security package was not recognized."
	case C.SEC_E_SECURITY_QOS_FAILED:
		s = "The security context could not be established due to a failure " +
			"in the requested quality of service"
	...
	case C.SEC_E_SMARTCARD_CERT_REVOKED:
		s = "The smart card certificate used for authentication has been revoked. " +
			"Additional information may exist in the event log."
	...
	case C.SEC_E_UNFINISHED_CONTEXT_DELETED:
		s = "A security context was deleted before the context was completed. " +
	...
	case C.SEC_E_UNSUPPORTED_PREAUTH:
		s = "An unsupported preauthentication mechanism was presented to " +
	...
	case C.SEC_E_WRONG_CREDENTIAL_HANDLE:
		s = "The supplied credential handle does not match the credential " +
	...
	case C.SEC_I_CONTEXT_EXPIRED:
		s = "The message sender has finished using the connection and has initiated " +
			"a shutdown."
