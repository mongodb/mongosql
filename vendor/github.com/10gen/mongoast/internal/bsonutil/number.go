...
func asInt32(v bsoncore.Value) (int32, error) {
	...
	case bsontype.Decimal128:
		...
		i64, ok := dec.Int64()
		if !ok {
			return 0, errors.Errorf("cannot convert decimal %v to an int32", dec.b)
		}
...
// AsDecimal gets the bson.Value as a Decimal.
func AsDecimal(v bsoncore.Value) Decimal {
	d, err := asDecimal(v)
	if err != nil {
		panic(err.Error())
	}
...
// AsDecimalOK gets the bson.Value as a Decimal.
func AsDecimalOK(v bsoncore.Value) (Decimal, bool) {
	d, err := asDecimal(v)
	if err != nil {
		return d, false
	}
...
func asInt64(v bsoncore.Value) (int64, error) {
	...
	case bsontype.Decimal128:
		...
		i64, ok := dec.Int64()
		if !ok {
			return 0, errors.Errorf("cannot convert decimal %v to an int64", dec.b)
		}
...
func asDecimal(v bsoncore.Value) (Decimal, error) {
	switch v.Type {
	case bsontype.Decimal128:
		return NewDecimal(v.Decimal128())
	case bsontype.Int32, bsontype.Int64:
		...
		_ = d.SetMantScale(int64V, 0)
		return Decimal{d}, nil
	case bsontype.Double:
		doubleV := v.Double()
		d := decimal.WithContext(decimal.Context128)
		_ = d.SetFloat64(doubleV)
		return Decimal{d}, nil
	}

	return Decimal{}, errors.Errorf("cannot convert %v to a Decimal128", v)
}
...
func asFloat64(v bsoncore.Value) (float64, error) {
	...
	case bsontype.Decimal128:
		...
		f64, ok := dec.Float64()
		if !ok {
			return 0, errors.Errorf("cannot convert decimal %v to a float64", dec.b)
		}
