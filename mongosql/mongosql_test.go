	...
	tcases := []struct {
		...
		},
		{
			...
			expectedOutput: `[{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "DuAl"},"tableAlias": ` +
				`{"$literal": "DuAl"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "double"},"value": "$a"}],"_id": {"$numberInt":"0"}}}]`,
		...
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$match": {"$expr": false}},{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$a"}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   "INFORMATION_SCHEMA",
			expectedCollection: "DUAL",
		},
		{
			desc:               "empty pipeline for DUAL because of falsy filter",
			query:              "select 1+2 from DUAL where 1 = 2",
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$match": {"$expr": false}},{"$project": {"values": [{"database": {"$literal": null},"table": {"$literal": null},"tableAlias": {"$literal": null},"column": {"$literal": null},"columnAlias": {"$literal": "1+2"},"bsonType": {"$literal": "long"},"value": {"$literal": {"$numberLong":"3"}}}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   "INFORMATION_SCHEMA",
			expectedCollection: "DUAL",
		},
		{
		...
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$match": {"$expr": false}},{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$a"},{"database": {"$literal": "test"},"table": {"$literal": "bar"},"tableAlias": {"$literal": "bar"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$__joined_bar.a"}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   "INFORMATION_SCHEMA",
			expectedCollection: "DUAL",
		},
		{
		...
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$project": {"values": [{"database": {"$literal": "test2"},"table": {"$literal": "bar"},"tableAlias": {"$literal": "bar"},"column": {"$literal": "b"},"columnAlias": {"$literal": "b"},"bsonType": {"$literal": "long"},"value": "$b"}],"_id": {"$numberInt":"0"}}}]`,
		...
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$a"},{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "b"},"columnAlias": {"$literal": "b"},"bsonType": {"$literal": "long"},"value": "$b"},{"database": {"$literal": "test"},"table": {"$literal": null},"tableAlias": {"$literal": null},"column": {"$literal": null},"columnAlias": {"$literal": "a+b"},"bsonType": {"$literal": "double"},"value": {"$add": ["$a","$b"]}}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   testDBName,
			expectedCollection: "foo",
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$match": {"$expr": {"$and": [{"$gt": ["$c",null]},{"$gt": ["$b",null]},{"$lt": ["$b","$c"]}]}}},{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$a"}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   testDBName,
			expectedCollection: "foo",
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "t"},"column": {"$literal": "a"},"columnAlias": {"$literal": "my_a"},"bsonType": {"$literal": "long"},"value": "$a"},{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "t"},"column": {"$literal": "b"},"columnAlias": {"$literal": "b_cool"},"bsonType": {"$literal": "long"},"value": "$b"},{"database": {"$literal": "test"},"table": {"$literal": null},"tableAlias": {"$literal": null},"column": {"$literal": null},"columnAlias": {"$literal": "sum"},"bsonType": {"$literal": "double"},"value": {"$add": ["$a","$b"]}}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   testDBName,
			expectedCollection: "foo",
		},
		{
			...
			expectedOutput:     `[{"$match": {"a": {"$ne": null}}},{"$lookup": {"from": {"db": "test","coll": "bar"},"localField": "a","foreignField": "a","as": "__joined_bar"}},{"$unwind": "$__joined_bar"},{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "A"},"columnAlias": {"$literal": "A"},"bsonType": {"$literal": "long"},"value": "$A"},{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$a"},{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "b"},"columnAlias": {"$literal": "b"},"bsonType": {"$literal": "long"},"value": "$b"},{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "c"},"columnAlias": {"$literal": "c"},"bsonType": {"$literal": "long"},"value": "$c"},{"database": {"$literal": "test"},"table": {"$literal": "bar"},"tableAlias": {"$literal": "bar"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$__joined_bar.a"},{"database": {"$literal": "test"},"table": {"$literal": "bar"},"tableAlias": {"$literal": "bar"},"column": {"$literal": "d"},"columnAlias": {"$literal": "d"},"bsonType": {"$literal": "long"},"value": "$__joined_bar.d"}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   testDBName,
			expectedCollection: "foo",
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$lookup": {"from": {"db": "test2","coll": "bar"},"let": {},"pipeline": [],"as": "__joined_bar"}},{"$unwind": "$__joined_bar"},{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "A"},"columnAlias": {"$literal": "A"},"bsonType": {"$literal": "long"},"value": "$A"},{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$a"},{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "b"},"columnAlias": {"$literal": "b"},"bsonType": {"$literal": "long"},"value": "$b"},{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "c"},"columnAlias": {"$literal": "c"},"bsonType": {"$literal": "long"},"value": "$c"},{"database": {"$literal": "test2"},"table": {"$literal": "bar"},"tableAlias": {"$literal": "bar"},"column": {"$literal": "b"},"columnAlias": {"$literal": "b"},"bsonType": {"$literal": "long"},"value": "$__joined_bar.b"}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   testDBName,
			expectedCollection: "foo",
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$project": {"values": [{"database": {"$literal": null},"table": {"$literal": null},"tableAlias": {"$literal": null},"column": {"$literal": null},"columnAlias": {"$literal": "1+2"},"bsonType": {"$literal": "long"},"value": {"$literal": {"$numberLong":"3"}}}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   testDBName,
			expectedCollection: "foo",
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$group": {"_id": {},"count(*)": {"$sum": {"$numberInt":"1"}}}},{"$project": {"values": [{"database": {"$literal": null},"table": {"$literal": null},"tableAlias": {"$literal": null},"column": {"$literal": null},"columnAlias": {"$literal": "count(*)"},"bsonType": {"$literal": "long"},"value": "$count(*)"}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   testDBName,
			expectedCollection: "foo",
		},
		{
		...
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "A"},"columnAlias": {"$literal": "A"},"bsonType": {"$literal": "long"},"value": "$A"},{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$a"}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   testDBName,
			expectedCollection: "foo",
		},
		{
		...
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$lookup": {"from": {"db": "test","coll": "FOO"},"let": {},"pipeline": [],"as": "__joined_FOO"}},{"$unwind": "$__joined_FOO"},{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$a"},{"database": {"$literal": "test"},"table": {"$literal": "FOO"},"tableAlias": {"$literal": "FOO"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$__joined_FOO.a"}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   testDBName,
			expectedCollection: "foo",
		},
		{
		...
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$sort": {"a": {"$numberInt":"1"}}},{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "a"},"columnAlias": {"$literal": "A"},"bsonType": {"$literal": "long"},"value": "$a"},{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "A"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$A"}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   testDBName,
			expectedCollection: "foo",
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "mycte"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$a"}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   testDBName,
			expectedCollection: "foo",
		},
		{
		...
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$lookup": {"from": {"db": "test","coll": "foo"},"let": {},"pipeline": [],"as": "__joined_foo"}},{"$unwind": "$__joined_foo"},{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "FOO"},"tableAlias": {"$literal": "FOO"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$a"},{"database": {"$literal": "test"},"table": {"$literal": "FOO"},"tableAlias": {"$literal": "FOO"},"column": {"$literal": "b"},"columnAlias": {"$literal": "b"},"bsonType": {"$literal": "long"},"value": "$b"},{"database": {"$literal": "test"},"table": {"$literal": "FOO"},"tableAlias": {"$literal": "FOO"},"column": {"$literal": "c"},"columnAlias": {"$literal": "c"},"bsonType": {"$literal": "long"},"value": "$c"},{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"bsonType": {"$literal": "long"},"value": "$__joined_foo.a"}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   testDBName,
			expectedCollection: "FOO",
		},
		{
