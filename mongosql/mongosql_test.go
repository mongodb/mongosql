...
`))
	...
	tcases := []struct {
		...
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$match": {"$expr": false}},{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"value": "$a"}],"_id": {"$numberInt":"0"}}}]`,
		...
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$match": {"$expr": false}},{"$project": {"values": [{"database": {"$literal": null},"table": {"$literal": null},"tableAlias": {"$literal": null},"column": {"$literal": null},"columnAlias": {"$literal": "1+2"},"value": {"$literal": {"$numberLong":"3"}}}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   "INFORMATION_SCHEMA",
			expectedCollection: "DUAL",
		},
		{
		...
		},
		{
			...
			dbName:             testDBName,
			ctlg:               testCatalogWithoutSharding,
			expectedOutput:     `[{"$match": {"$expr": false}},{"$project": {"values": [{"database": {"$literal": "test"},"table": {"$literal": "foo"},"tableAlias": {"$literal": "foo"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"value": "$a"},{"database": {"$literal": "test"},"table": {"$literal": "bar"},"tableAlias": {"$literal": "bar"},"column": {"$literal": "a"},"columnAlias": {"$literal": "a"},"value": "$__joined_bar.a"}],"_id": {"$numberInt":"0"}}}]`,
			expectedDatabase:   "INFORMATION_SCHEMA",
			expectedCollection: "DUAL",
		},
		{
