...
func TestTranslateSQLQuery(t *testing.T) {
	tcases := []struct {
		...
		},
		{
			...
			schema:         testSchema,
			format:         testFormat,
			expectedOutput: `[{"$match": {"$expr": {"$and": [{"$and": [{"$lt": ["$b","$c"]},{"$gt": ["$b",null]}]},{"$gt": ["$c",null]}]}}},{"$project": {"test_DOT_foo_DOT_a": "$a","_id": NumberInt("0")}}]`,
		},
		{
		...
		},
		{
			...
			schema:         testSchema,
			format:         "multiline",
			expectedOutput: "[\n\t{\"$lookup\": {\"from\": \"baz\",\"let\": {\"local_table__a\": \"$a\"},\"pipeline\": [{\"$match\": {\"$expr\": {\"$and\": [{\"$and\": [{\"$gt\": [\"$$local_table__a\",\"$b\"]},{\"$gt\": [\"$$local_table__a\",null]}]},{\"$gt\": [\"$b\",null]}]}}}],\"as\": \"__joined_baz\"}},\n\t{\"$unwind\": \"$__joined_baz\"},\n\t{\"$project\": {\"test_DOT_foo_DOT_a\": \"$a\",\"test_DOT_baz_DOT_b\": \"$__joined_baz.b\",\"_id\": NumberInt(\"0\")}},\n]",
		},
		{
