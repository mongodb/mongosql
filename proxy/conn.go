...
func (c *Conn) Handshake() error {
	if err := c.writeInitialHandshake(); err != nil {
		log.Logf(log.Always, "send initial handshake error %s", err.Error())
		return err
	}

	if err := c.readHandshakeResponse(); err != nil {
		log.Logf(log.Always, "recv handshake response error %s", err.Error())
	...
	if err := c.writeOK(nil); err != nil {
		log.Logf(log.Always, "write ok fail %s", err.Error())
		return err
	}
...
func (c *Conn) readHandshakeResponse() error {
	...
	checkAuth := CalcPassword(c.salt, []byte(c.server.opts.SQLPassword))
