...
test-atlas-connect-success: build-mongosqld run-mongosqld _test-connect-success _test-find-doc
_test-find-doc: QUERY := SELECT message FROM test.greeting WHERE _id = '5c64a48d1c9d44000046008d'
_test-find-doc: EXPECTED := Hello, world!
_test-find-doc: _test-mysql-query
...
# is different from prior mongodb versions since 3.7+ requires authentication to
# list all databases.
test-mongo-auth-sample-no-creds-4.2: INFRASTRUCTURE_CONFIG := $(INFRASTRUCTURE_CONFIG),mongo/auth,mongo/version/4.2,sqlproxy/schema/mapping-majority
test-mongo-auth-sample-no-creds-4.2: test-auth-schema-not-available
...
# when incorrect admin credentials are provided, we expect the connection to fail
# because the schema is not yet available
test-mongo-auth-sample-wrong-admin-creds-4.2: INFRASTRUCTURE_CONFIG := $(INFRASTRUCTURE_CONFIG),mongo/auth,mongo/version/4.2,sqlproxy/schema/mapping-majority,sqlproxy/auth/wrong-admin-creds,sqlproxy/auth/enabled,sqlproxy/ssl/allow,sqlproxy/ssl/pem,client/auth/cleartext,client/ssl/require,client/auth/creds
test-mongo-auth-sample-wrong-admin-creds-4.2: test-auth-schema-not-available
...
# when correct admin credentials are provided but the user has no privileges,
# we expect the connection to fail because the schema is not yet available
test-mongo-auth-sample-no-roles-4.2: INFRASTRUCTURE_CONFIG := $(INFRASTRUCTURE_CONFIG),mongo/auth,mongo/version/4.2,mongo/other-user/no-roles,sqlproxy/schema/mapping-majority,sqlproxy/auth/admin-creds-other-user,sqlproxy/auth/enabled,sqlproxy/ssl/require,sqlproxy/ssl/pem,client/auth/cleartext,client/ssl/require,client/auth/creds
test-mongo-auth-sample-no-roles-4.2: test-auth-empty-schema-available
...
# when correct admin credentials are provided but the user does not have the listDatabases action,
# we expect the connection to succeed as long as we are running against 3.7+, even when wildcards are used in db selectors
test-mongo-auth-sample-no-listdatabases-wildcard-db-4.2: INFRASTRUCTURE_CONFIG := $(INFRASTRUCTURE_CONFIG),mongo/auth,mongo/other-user/no-listdatabases,sqlproxy/schema/mapping-majority,sqlproxy/schema/ns-wildcard-db,sqlproxy/auth/admin-creds-other-user,sqlproxy/auth/enabled,sqlproxy/ssl/allow,sqlproxy/ssl/pem,client/auth/cleartext,client/ssl/require,client/auth/creds
test-mongo-auth-sample-no-listdatabases-wildcard-db-4.2: NO_FLUSH_SCHEMA := 1
test-mongo-auth-sample-no-listdatabases-wildcard-db-4.2: EXPECTED_DB_COUNT := 4
test-mongo-auth-sample-no-listdatabases-wildcard-db-4.2: test-auth-schema-available

# when correct admin credentials are provided but the user does not have the listDatabases action,
# we expect the connection to succeed as long as we are running against 3.7+, even when wildcards are used in db selectors
test-mongo-auth-sample-no-listdatabases-wildcard-db-4.4: INFRASTRUCTURE_CONFIG := $(INFRASTRUCTURE_CONFIG),mongo/auth,mongo/other-user/no-listdatabases,sqlproxy/schema/mapping-majority,sqlproxy/schema/ns-wildcard-db,sqlproxy/auth/admin-creds-other-user,sqlproxy/auth/enabled,sqlproxy/ssl/allow,sqlproxy/ssl/pem,client/auth/cleartext,client/ssl/require,client/auth/creds
test-mongo-auth-sample-no-listdatabases-wildcard-db-4.4: NO_FLUSH_SCHEMA := 1
test-mongo-auth-sample-no-listdatabases-wildcard-db-4.4: EXPECTED_DB_COUNT := 4
test-mongo-auth-sample-no-listdatabases-wildcard-db-4.4: test-auth-schema-available
