...
set -v
mongoexport --host bi-beta.mongodb.com --type=csv --fields="carrier_code,tail_num,flight_number,origin_airport_id,origin_city_market_id,origin_airport_code,origin_city_name,origin_state,dest_airport_id,dest_city_market_id,dest_airport_code,dest_city_name,dest_state,dest_time,diff_from_dep_time,dep_delay,arr_time,diff_from_arr_time,airline,cancelled,arr_delay,carrier_delay" -d tableau -c flights201406 > flights.csv;
mongoexport --host bi-beta.mongodb.com --type=csv --fields="loc.coordinates.0,loc.coordinates.1,airport_city,airport_code,airport_id,city,city_market_description,city_market_id,country,state,zip" -d tableau -c attendees > attendees.csv;
mysql -u root -e 'DROP TABLE IF EXISTS flights' test
